...
export class HaVoiceAssistantSetupStepLocal extends LitElement {
  ...
  private async _checkLocal() {
    ...
    try {
      ...
      const ttsAddon = addons.find(addon => addon.slug === this._ttsAddonName)
      const sttAddon = addons.find(addon => addon.slug === this._sttAddonName)
  ...
  private async _createPipeline(ttsEntityId: string, sttEntityId: string) {
    ...
    if (!agent?.supported_languages.length) {
      throw new Error('Conversation agent does not support requested language.')
    }
