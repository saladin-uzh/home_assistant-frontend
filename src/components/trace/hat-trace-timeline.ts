...
class ActionRenderer {
  ...
  private _handleChoose(index: number): number {
    ...
    const chooseConfig = this._getDataFromPath(this.keys[index]) as ChooseAction
  ...
  private _handleRepeat(index: number): number {
    ...
    const repeatConfig = this._getDataFromPath(this.keys[index]) as RepeatAction
...
export class HaAutomationTracer extends LitElement {
  ...
  @property({ attribute: 'allow-pick', type: Boolean }) public allowPick = false
