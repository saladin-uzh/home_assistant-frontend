      ...
      Object.entries(filters).forEach(([key, filter]) => {
        if (
          ...
          const entryIds = entries
            .filter(entry => (filter.value as string[]).includes(entry.domain))
          ...
          outputDevices = outputDevices.filter(device =>
            device.labels.some(lbl => (filter.value as string[]).includes(lbl))
          )
  ...
  private _columns = memoizeOne((localize: LocalizeFunc) => {
    ...
    return {
      ...
      labels: {
        ...
        hidden: true,
        filterable: true,
        template: device => device.label_entries.map(lbl => lbl.name).join(' '),
      },
        ...
        updateDeviceRegistryEntry(this.hass, deviceId, {
          labels:
            action === 'add'
              ...
              : this.hass.devices[deviceId].labels.filter(lbl => lbl !== label),
        })
      )
    })
    const result = await Promise.allSettled(promises)
